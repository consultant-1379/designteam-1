...
import com.ericsson.eniq.asn1kafkaconsumer.cache.TransformerCacheImpl;
import com.ericsson.eniq.parser.cache.DBLookupCache;


@SpringBootApplication
public class Asn1kafkaconsumerApplication implements ApplicationListener<KafkaEvent> {
	
	@Autowired
	private Environment env;

	public static void main(String[] args) {
		SpringApplication.run(Asn1kafkaconsumerApplication.class, args);
	}
	...
	public ApplicationRunner runner(KafkaListenerEndpointRegistry registry,
		...
		return args -> {
			...
			registry.getListenerContainer("#{'${spring.kafka.consumer.group-id}'}").pause();
	    	...
	    	dbread.readDB(repdbUrl, 
	    			dwhrepUser, dwhrepPass, repDbDriver,"dwhrep", "DC_E_BSS"); 
	    	
			DataFormatCacheImpl dataformats = new DataFormatCacheImpl();
	    	dataformats.readDB(repdbUrl, 
	    			dwhrepUser, dwhrepPass, repDbDriver, "DC_E_BSS");
	    	  	
	    	
	    	System.out.println("START UP SQUENCE COMPLETE!");
	    	registry.getListenerContainer("#{'${spring.kafka.consumer.group-id}'}").resume();
