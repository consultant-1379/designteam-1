...
public class DataFormatCache {
	...
	final Map<String, Map<String, DFormat>> interfaceTagIdLookUpMap = new ConcurrentHashMap<>();
	...
	public void addDataFormat(String dataformatID, String tagID, String interfaceName, String folderName,
		...
		if (dataformats.containsKey(dataformatID)) {
			...
			dataformat.setInterfaceName(interfaceName);
			dataformat.setTagID(tagID);
			dataformat.setTransformerID(transformerID);
		} else {
			dataformat = new DFormat(interfaceName, tagID, dataformatID, folderName, transformerID);
		}

		dataformats.put(dataformatID, dataformat);
		tagIdLookUpMap.put(tagID, dataformat);
		Map<String, DFormat> tagIdMap = interfaceTagIdLookUpMap.get(interfaceName);
		if (tagIdMap == null) {
			tagIdMap = new HashMap<>();
			interfaceTagIdLookUpMap.put(interfaceName, tagIdMap);
		}
		tagIdMap.put(tagID, dataformat);
	...
	public DFormat getDataFormatByInterfaceAndTagId(String interfaceName, String tagId) {
		Map<String, DFormat> tagIdMap = interfaceTagIdLookUpMap.get(interfaceName);
		if(tagIdMap == null) {
			return null;
		}
