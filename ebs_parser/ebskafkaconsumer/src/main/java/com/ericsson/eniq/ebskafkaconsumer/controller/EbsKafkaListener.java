...
import com.ericsson.eniq.ebskafkaconsumer.controller.pojo.ParserInput;
...
import com.ericsson.eniq.parser.EBSParser.EBSParser;
import com.ericsson.eniq.parser.sink.ISink;
import com.google.gson.Gson;

@Component
public class EbsKafkaListener {

	private static final Logger logger = LogManager.getLogger(EbsKafkaListener.class);
	...
	@KafkaListener(id = "PM_E_EBS_FILES_GROUP", topics = "PM_E_EBS_FILES", containerFactory = "batchFactory")
	...
	private void createWorker(String message, int producerIndex) {
		...
		EBSParser parser = new EBSParser(sf, techpack, setType, setName, "ebs_worker", sinks.get(producerIndex));
		workers.add(parser);
	}
	...
	public void createKafkaProducers(ApplicationContext context) {
		...
		for (int i=1 ; i <= bs ; i++) {
			sinks.put(i, new EbsKafkaProducer((KafkaTemplate<String, GenericRecord>)context.getBean(KafkaTemplate.class), producerTopic));
			//sinks.put(i, new SbKafkaTextProducer((KafkaTemplate<String, String>)context.getBean(KafkaTemplate.class)));
		}
		logger.log(Level.INFO, "Sinks = " + sinks);
	}
