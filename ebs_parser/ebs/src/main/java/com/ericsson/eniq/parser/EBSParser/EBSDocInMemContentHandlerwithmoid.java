 ...
 * 
 */
public class EBSDocInMemContentHandlerwithmoid extends DefaultHandler {
 ...
 //...OTHER INFO
  ...
  public EBSDocInMemContentHandlerwithmoid() {
    super();

  }

  public EBSDocInMemContentHandlerwithmoid(final SourceFile sf, final String techPack,
...
//    MeasurementFile mFile = null;
    ...
    for(String moidKey : documentData.keySet()){
     ...
     try {
       ...
       measFile.pushData(moidData);
       //measFile.saveData();
    } catch (Exception e) {
      log.log(Level.WARN,"Failed to save measurementFile with MOID="+moidKey);
      e.printStackTrace();
    } 
...
//        measValueMap = new HashMap<String, String>();//This is probably not needed
      ...
      log.log(Level.TRACE,"StartMV: Initializing measValueMap");
  ...
  public void endElement(final String uri, final String name, final String qName)
   ...
   if (TAG_SN.equals(qName)) {
      ...
      log.log(Level.TRACE, "Tag parsed: "+TAG_SF+"="+suspectFlag);
      ...
      try {
        ...
        for (String measNameIndexKey : measNameMap.keySet()) {
          ...
          log.log(Level.TRACE, "moidData.put( "
...
private void addMoidDataToDocumentData(String moid, Map<String,String> moidData, Map<String, Map<String,String>> documentData) {
     ...
     if(moid!=null && !"".equals(moid) && moidData!=null){
      ...
      if(documentData==null){
        ...
        log.log(Level.INFO,"Created files-map that contains all measurement files with moid as a key");
      }
      ...
      if(documentData.containsKey(moid)){
        ...
        log.log(Level.INFO,"mapData (size="+moidData.size()+") added to existing data");
      }else{
        //File with given moid does not exist so we can put this as a new one
        documentData.put(moid, moidData);
        log.log(Level.INFO,"mapData (size="+moidData.size()+") added to NEW measurement file ("+moidData.toString()+")");
      }
     }else{
  ...
  String handleMoid(String moid, boolean fillEmptyMoid, String fillEmptyMoidStyle, String fillEmptyMoidValue, String measValueIndex){
    ...
    if (fillEmptyMoid && (moid==null || "".equals(moid)) ) {
      if (fillEmptyMoidStyle.equalsIgnoreCase("static")) {
        ...
        log.log(Level.INFO,"Filling empty moid with static value="+fillEmptyMoidValue);
      } 
      else {
        moid = measValueIndex + "";//##TODO## Is this still valid?
        log.log(Level.INFO,"Filling empty moid with inc value="+measValueIndex);
      }
    }
    if(moid==null){
      moid = "";//Return empty string instead of null
      log.info("handled Moid is empty String. " +
  ...
  /*private MeasurementFile createNewMeasurementFile() throws SAXException {
    log.log(Level.INFO,"Trying to create new measurement file");
    try {
      if (sourceFile != null) {
        ...
        if (oldObjClass == null || !oldObjClass.equals(objectClass)) {
          ...
          if (measFile != null) {
            //measFile.close();
            log.log(Level.INFO,"Closed a measurement file");
          }
          // create new measurementFile
          log.log(Level.INFO,"Trying to create new measurementFile with objectClass="+objectClass);
          measFile = getChannel(objectClass);
          oldObjClass = objectClass;
          
          log.log(Level.INFO,"New measurement file created");
        }
      }
    } catch (Exception e) {
