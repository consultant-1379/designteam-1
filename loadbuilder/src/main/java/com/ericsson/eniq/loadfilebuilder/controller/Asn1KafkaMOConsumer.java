...
import com.ericsson.eniq.loadfilebuilder.outputstream.Data;
...
@Service
public class Asn1KafkaMOConsumer {

	private static final Logger LOG = LogManager.getLogger(Asn1KafkaMOConsumer.class);
	...
	@KafkaListener(id = "PM_E_BSS_DATA_GROUP", topics = "PM_E_BSS_DATA", containerFactory = "batchFactory")
	void listen(ConsumerRecords<String, GenericRecord> consumerRecords, Acknowledgment acknowledgment) {
		...
		for (ConsumerRecord<String, GenericRecord> consumerRecord : consumerRecords) {
			...
			// LOG.log(Level.INFO, "Record details : partitiion = " +
			// consumerRecord.partition() + " ,offset = "
